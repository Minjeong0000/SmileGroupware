...
public interface MessageMapper {
    ...
    //받은쪽지 조회
    @Select("""
            SELECT MU_RECEIVER.MESSAGE_USER_NO, M.MESSAGE_NO, MU_RECEIVER.FORDER_NO, F.FORDER_NAME, M.CONTENT, M.SENT_AT, M.SENDER_NO, S.EMP_NAME AS SENDER_NAME, M.RECEIVER_NO, R.EMP_NAME AS RECEIVER_NAME, MU_RECEIVER.READ_YN
            FROM MESSAGE M
            JOIN EMPLOYEE S ON M.SENDER_NO = S.EMP_ID
            JOIN EMPLOYEE R ON M.RECEIVER_NO = R.EMP_ID
            LEFT JOIN MESSAGE_USER MU_RECEIVER ON M.MESSAGE_NO = MU_RECEIVER.MESSAGE_NO
            AND MU_RECEIVER.EMP_ID = R.EMP_ID JOIN FORDER F ON MU_RECEIVER.FORDER_NO = F.FORDER_NO
            WHERE R.EMP_ID = #{empId}
            AND MU_RECEIVER.EMP_ID = #{empId}
            AND MU_RECEIVER.FORDER_NO!=2
    ...
    //중요쪽지 조회
    @Select("""
                  SELECT M.MESSAGE_NO, MU.MESSAGE_USER_NO, M.SENDER_NO, S.EMP_NAME AS SENDER_NAME, M.RECEIVER_NO, R.EMP_NAME AS RECEIVER_NAME, M.CONTENT, M.SENT_AT, MU.READ_YN, MU.DELETED_DATE, F.FORDER_NO
                  FROM MESSAGE M
                  JOIN MESSAGE_USER MU ON M.MESSAGE_NO = MU.MESSAGE_NO
                  AND MU.EMP_ID = #{empId}
                  JOIN EMPLOYEE S ON M.SENDER_NO = S.EMP_ID
                  JOIN EMPLOYEE R ON M.RECEIVER_NO = R.EMP_ID
                  JOIN FORDER F ON MU.FORDER_NO = F.FORDER_NO
                  WHERE MU.FORDER_NO = 1 ORDER BY M.MESSAGE_NO DESC
    """)
    ...
    //보낸쪽지함
    @Select("""
             SELECT MU_SENDER.MESSAGE_USER_NO, M.MESSAGE_NO, M.CONTENT, M.SENT_AT, R.EMP_NAME AS RECEIVER_NAME, M.RECEIVER_NO, S.EMP_NAME AS SENDER_NAME, M.SENDER_NO, MU_RECEIVER.READ_YN,MU_SENDER.FORDER_NO
             FROM MESSAGE M JOIN EMPLOYEE S ON M.SENDER_NO = S.EMP_ID
             JOIN EMPLOYEE R ON M.RECEIVER_NO = R.EMP_ID
             JOIN MESSAGE_USER MU_SENDER ON M.MESSAGE_NO = MU_SENDER.MESSAGE_NO
             AND MU_SENDER.EMP_ID = S.EMP_ID
             LEFT JOIN MESSAGE_USER MU_RECEIVER ON M.MESSAGE_NO = MU_RECEIVER.MESSAGE_NO
             AND MU_RECEIVER.EMP_ID = R.EMP_ID
             WHERE S.EMP_ID = #{empId}
             AND MU_SENDER.EMP_ID = #{empId}
             AND MU_SENDER.FORDER_NO !=2 ORDER BY M.MESSAGE_NO DESC
    """)
